...
if [[ $# -lt 5 ]]; then
    echo "Usage: $0 <device group> <hardware ID> <build ID file 1> <build ID file 2> <build ID file 3> <build ID file 4"
    exit 1
fi
...
elif [[ $current = $build_c ]]; then
    runtime job fwup -g $dg -d $hwid -i $build_d -w --timeout=20m
else
